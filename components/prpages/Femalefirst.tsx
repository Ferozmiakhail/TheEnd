...
<div className="product-detail-container">
  ...
  <div className="product-detail-desc">
          ...
          <div className='more-prob more-prob-thick'><h4 className='more-prob-thick detailsallchange'>{ Sdetails }</h4><p onClick={() => setmoredetail(true)} className='Readmore moreallchange'> ...Full-details</p></div>
          {moredetail &&
  <div className="Size-wrapper">
    ...
    <div>  
      <button className={toSizeSelect === 'medium' ? 'info-size-selecter info-size-selecter-clicked info-size-slector-cliked-prob' : 'info-size-selecter info-size-slector-cliked-prob'} onClick={() => {settoSizeSelect('medium'); setnosizewarn(false);}}>Free Size</button>
      {donotbuyforcoat && <><div className='req-warn'>Please select your Jewellery</div></>}
      {hidejew && <><div onClick={() => {if(isjew == true){ setisjew(false); setdonotbuyforcoat(false); }else{ setnosizewarn(false); setisjew(true);}}} className='coat-probab'><input type="checkbox" checked={isjew? true : false} /><p className='prefer-prob'>Prefer jewellery along</p></div>
      ...
      <div className='sizing-shower-detail sizing-shower-detail-Two'>
        {toSizeSelect === 'medium'&&
          <>
          <div className='flexer'><p className='f1'>Lenght: </p><p className='f2'>{"21"}</p></div>
          ...
          <div className='flexer'><p className='f1'>Pant length: </p><p className='f2'>{"38"}</p></div>
          </>
        }
          </div>
            </div>
    ...
    <div className="maylike-products-wrapper">
        ...
        <div className="marquee">
          <div className="maylike-products-container track">
            {products.map((item) => (
              item.productcategory === "Femalefirst"? <Product image={item.image}
              name={item.name}
