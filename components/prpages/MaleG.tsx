...
interface Props {
  ...
  sSkirt: string,
   ...
   maleSizeG: boolean, 
   setmaleSizeG: (maleSizeG: boolean) => void,
   gethip: number,
   getblouse: number,
   getdotpoint: number,
   handleBuyNowM: any,
...
<div className="product-detail-container">
  ...
  <div className="Size-wrapper">
    ...
    <div>  
        ...
        <button className={toSizeSelect === 'mine' ? 'info-size-selecter info-size-selecter-clicked info-size-slector-cliked-prob' : 'info-size-selecter info-size-slector-cliked-prob'} onClick={() => {if(getstok < 1 || getshoulder < 1 || getsleeve < 1 || getcollar < 1 || getchest < 1 || getpant < 1 || getlast < 1 || pant !== 'pant1' && pant !== 'pant2' && pant !== 'pant3' && pant !== 'pant4'){ setWarnmine(true); settoSizeSelect('Nothing')}else if(getstok >= 1 && getshoulder >= 1 && getsleeve >= 1 && getcollar >= 1 && getchest >= 1 && getpant >= 1 && getlast >= 1){ setWarnmine(false); settoSizeSelect('mine'); setnosizewarn(false);}}}>My size</button>
        {/* <button onClick={() => {setmaleSizeG(true); setsizemPage('p1')}} className={Warnmine ? 'info-size-selecter bg-greep' : 'info-size-selecter bg-green'}>Custom Size</button> */}
        <button onClick={() => {if(iscoat){if(hidecoat){setdonotbuyforcoat(true);}else{setmaleSizeG(true); setsizemPage('p1')}}else{setmaleSizeG(true); setsizemPage('p1')}}} className={Warnmine ? 'info-size-selecter bg-greep' : 'info-size-selecter bg-green'}>Custom Size</button>
    ...
    <div className="maylike-products-wrapper">
        ...
        <div className="marquee">
          <div className="maylike-products-container track">
            {products.map((item) => (
              item.productcategory === "MaleG"? <Product
              key={item._id}
              image={item.image}
              name={item.name}
